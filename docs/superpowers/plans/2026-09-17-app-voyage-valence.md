...
**Goal:** Livrer une PWA Nuxt 3 installable, 100% fonctionnelle hors ligne après une première visite en ligne, qui présente à l'utilisatrice une carte de Valence, un programme d'événements, un guide de lieux, des messages surprise verrouillés dans le temps, et des phrases utiles en espagnol.

**Architecture:** SPA Nuxt 3 (`ssr: false`) avec 5 pages sous une barre d'onglets fixe. L'état applicatif tient dans des composables Vue légers (pas de store global) persistés via `localStorage`. Le contenu du voyage vit dans un unique fichier `data/content.ts`. La carte hors ligne combine un extrait `.pmtiles` de Valence (protocole `pmtiles` + MapLibre GL) avec un style généré localement via `@protomaps/basemaps`, dont toutes les ressources (police, sprites) sont hébergées dans `public/`. Le service worker (`@vite-pwa/nuxt`) précache l'app shell ; le `.pmtiles` est mis en cache séparément via une règle Workbox à range requests plus un téléchargement explicite au démarrage.
...
**Spec:** `docs/superpowers/specs/2026-09-17-app-voyage-valence-design.md`
...
git commit -m "feat: add trip content data model with placeholder Valencia content"
```
...
**Files:**
- Create: `public/valencia.pmtiles` (binaire)
...
- [ ] **Step 3: Extraire la bbox de Valence**
...
Cette bbox couvre le centre historique (Ciutat Vella), la Ciutat de les Arts i les Ciències et la plage de la Malvarrosa.

- [ ] **Step 4: Vérifier l'archive extraite**

Run: `pmtiles verify public/valencia.pmtiles`
Expected: aucune erreur signalée.

Run: `ls -lh public/valencia.pmtiles`
...
```bash
git add public/valencia.pmtiles public/style scripts/build-map-data.md
git commit -m "feat: add offline Valencia map tiles, fonts and sprites"
```
...
**Interfaces:**
- Produces: `creerStyleValence(origin: string): StyleSpecification`. Consommé par Task 21 (composant carte).
...
## Task 21: Composant `MapaValencia.client.vue`

**Files:**
- Create: `components/MapaValencia.client.vue`

**Interfaces:**
- Consumes: `lieux` (Task 4), `creerStyleValence` (Task 20, auto-importé), `usePosition` (Task 8, auto-importé), `useMapCache` (Task 10, auto-importé), `useMapFocus` (Task 9, auto-importé).
- Produces: composant `<MapaValencia>` auto-enregistré par Nuxt (suffixe `.client.vue` : rendu navigateur uniquement, jamais lors du prerendering).

- [ ] **Step 1: Implémenter `components/MapaValencia.client.vue`**
...
```bash
git add components/MapaValencia.client.vue
...
**Interfaces:**
- Consumes: `<MapaValencia>` (Task 21, auto-enregistré).
...
Run: `test -f .output/public/sw.js && grep -c "valencia" .output/public/sw.js`
...
Run: `test -f .output/public/valencia.pmtiles && test -f .output/public/manifest.webmanifest && test -f .output/public/sw.js && echo OK`
Expected: `OK`
...
Run: `npm run generate`
Expected: se termine sans erreur ; `.output/public` contient `index.html`, `carte/index.html`, `a-venir/index.html`, `guide/index.html`, `messages/index.html`, `phrases/index.html`, `valencia.pmtiles`, `manifest.webmanifest`, `sw.js`, `icons/icon-192.png`, `icons/icon-512.png`, `icons/apple-touch-icon.png`, `style/fonts/`, `style/sprites/`.
