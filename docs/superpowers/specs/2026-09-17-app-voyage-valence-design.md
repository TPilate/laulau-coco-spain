# App de voyage « Laulau & Coco » — Valence
...
Application web installable (PWA) offerte en cadeau pour un voyage de 5 jours à Valence
...
- Carte de Valence hors ligne (fichier `.pmtiles`) affichée avec MapLibre GL.
...
export const lieux: Lieu[] = [ /* ~6-8 exemples réalistes à Valence */ ]
...
Valence : ex. Ciutat de les Arts i les Ciències, Mercado Central, Malvarrosa, Barrio del
...
3. Extraire une bbox couvrant le centre historique (Ciutat Vella), la Ciutat de les Arts
   i les Ciències et la plage de la Malvarrosa, jusqu'au zoom 15 :
   ```bash
   pmtiles extract https://build.protomaps.com/<AAAAMMJJ>.pmtiles public/valencia.pmtiles \
...
// source : url: `pmtiles://${location.origin}/valencia.pmtiles`
```

Le composant carte (`MapaValencia.client.vue`) est client-only : le protocole `pmtiles`
...
2. **Téléchargement complet explicite** au premier lancement, dans le même cache
   ...
   if (!(await cache.match('/valencia.pmtiles'))) {
     await cache.add('/valencia.pmtiles')
   }
...
- SVG source unique, palette Valence (orange/rouge chaud type paella, accent bleu
...
3. Vérifier après déploiement que `valencia.pmtiles` est servi avec le support des range
